...
struct __bind {
  ...
  R operator()(_Args&&... args) {
    ...
    auto value_tuple = detail::xform_placeholder_tuple<T, __VTT(_Args)>::
...
__bind<R, R (*)(F_Args...), __VTT(Args)> {
  using namespace thrust;
  typedef R (*F)(F_Args...);

  return __bind<R, F, __VTT(Args)>
...
__bind<R, R (C::*)(F_Args...), __VTT(Args)> {
  using namespace thrust;
...
// Generic functors.
// Return-type introspection doesn't work for device
// lambdas since they are not monomorphic.
template<typename R, typename F, typename... Args>
__host__ __device__
auto bind(F&& f, Args&&... args) ->
__bind<R, F, __VTT(Args)> {
  return __bind<R, F, __VTT(Args)>
    (std::forward<F>(f), thrust::make_tuple(std::forward<Args>(args)...));
}

// Functors with nested result_type defined.
template<typename F, typename... Args>
__host__ __device__
auto bind(F&& f, Args&&... args) ->
__bind<typename F::result_type, F, __VTT(Args)> {
  return __bind<typename F::result_type, F, __VTT(Args)>
    (std::forward<F>(f), thrust::make_tuple(std::forward<Args>(args)...));
}
